...
```{r }
MatrCc	<- MRARegress (MatExp1)					# Perturb = NULL, NodeName = NULL, KnlgMap = NULL, Method = "TLR", MapExper = NULL, Verbose = FALSE (default values)
if (! is.null(MatrCc)) {
	r 	<- round(MatrCc$r, 3)					# MRARegress returns other values. See documentation for details.
	DrawGraph (r)
}
```
...
MatrCc	<- MRARegress (MatExp2, Perturb2)				# NodeName = NULL, KnlgMap = NULL, Method = "TLR", MapExper = NULL, Verbose = FALSE (default values)
														# MatExp2 has 10 rows and 21 columns. Perturb is mandatory.
if (! is.null(MatrCc)) {
	MatrCcDiscr	<- Classify(MatrCc)						# Classes = c(0,1), Method = "TLR", Lbda = NULL (default values)
	DrawDiscr (MatrCcDiscr, Solution)					# Draws the second image
}
```
